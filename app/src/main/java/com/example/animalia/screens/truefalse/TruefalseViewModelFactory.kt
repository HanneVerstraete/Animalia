...
class TruefalseViewModelFactory(val quizElementRepository: QuizElementRepository, val index: Int) :
    ViewModelProvider.Factory {
    override fun <T : ViewModel> create(modelClass: Class<T>): T {
        if (modelClass.isAssignableFrom(TruefalseViewModel::class.java)) {
            @Suppress("UNCHECKED_CAST")
            return TruefalseViewModel(quizElementRepository, index) as T
        }
