...
class LessonOverviewViewModel(val database: LessonDatabaseDao) :
    ViewModel() {
    val lessons = database.getAllLessonsAsLiveData()
    ...
    private suspend fun addLessonsToDatabase() {
        return withContext(Dispatchers.IO) {
            ...
            database.insert(newLesson0)
            database.insert(newLesson1)
            database.insert(newLesson2)
        }
    }
}